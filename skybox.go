...
func NewImageSkybox(path string, intensity float32) *ImageSkybox {
	f, err := os.Open(path)
	if err != nil {
		panic(err)
	}
...
func (s *ImageSkybox) Sample(direction Vec3) Vec3 {
	...
	x := int(u * float32(width-1))
	y := int(v * float32(height-1))
